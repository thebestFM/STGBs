...
class NeighborSampler:
    ...
    def get_first_order_historical_neighbors(self, node_ids: np.ndarray, node_interact_times: np.ndarray, num_neighbors: int = 32):
        """
    ...
    def get_multi_hop_neighbors(self, num_hops: int, node_ids: np.ndarray, node_interact_times: np.ndarray, num_neighbors: int = 20):
        ...
        nodes_neighbor_ids, nodes_edge_ids, nodes_neighbor_times = self.get_first_order_historical_neighbors(node_ids=node_ids,
        ...
        for hop in range(1, num_hops):
            ...
            nodes_neighbor_ids, nodes_edge_ids, nodes_neighbor_times = self.get_first_order_historical_neighbors(node_ids=nodes_neighbor_ids_list[-1].flatten(),
...
class NegativeEdgeSampler(object):
    ...
    def __init__(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, interact_times: np.ndarray = None, last_observed_time: float = None,
        ...
        if self.negative_sample_strategy == 'inductive':
            # sets of observed and all train and test edges
            self.observed_edges = self.get_unique_edges_between_start_end_time(self.earliest_time, self.last_observed_time)
            self.all_edges = self.get_unique_edges_between_start_end_time(self.earliest_time, max(self.unique_interact_times))
    ...
    def inductive_sample(self, size: int, batch_src_node_ids: np.ndarray, batch_dst_node_ids: np.ndarray,
                         current_batch_start_time: float, current_batch_end_time: float, last_observed_time):
        """
        ...
        # get edges in the current batch
        current_batch_edges = self.get_unique_edges_between_start_end_time(start_time=current_batch_start_time, end_time=current_batch_end_time)
        # According to the Fig.4 in 'Towards Better Evaluation for Dynamic Link Prediction', we correct the inductive sampling process,
        # focusing on observing the edges that were not seen during training
        unique_inductive_edges = self.all_edges - self.observed_edges - current_batch_edges
