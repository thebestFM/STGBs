...
class CalendarTimeEncoder(nn.Module):
    ...
    def __init__(self, time_dim: int, time_segment: int, calendar_base: str, parameter_requires_grad: bool = True):
        """
        CalendarTime encoder.
        :param time_dim: int, dimension of time encodings
        :param time_segment: int, number of time segments
        :param calendar_base: str, calendar base information
        :param parameter_requires_grad: boolean, whether the parameter in CalendarTimeEncoder needs gradient
        """
        super(CalendarTimeEncoder, self).__init__()

        self.time_dim = time_dim
        self.calendar_base = calendar_base
        
        # constant parameters for fine-granularity temporal encoding
        self.w = nn.Linear(1, time_dim)
        self.w.weight = nn.Parameter((torch.from_numpy(1 / 10 ** np.linspace(0, 9, time_dim, dtype=np.float32))).reshape(time_dim, -1))
        self.w.bias = nn.Parameter(torch.zeros(time_dim))

        # constant parameters for coarse-granularity temporal encoding
        self.w_calendar = nn.Linear(1, time_dim)
        self.w_calendar.weight = nn.Parameter(torch.ones(time_dim,1)/time_segment)
        self.w_calendar.bias = nn.Parameter(torch.zeros(time_dim))

        if not parameter_requires_grad:
            self.w.weight.requires_grad = False
            self.w.bias.requires_grad = False

            self.w_calendar.weight.requires_grad = False
            self.w_calendar.bias.requires_grad = False
    ...
    def forward(self, timestamps: torch.Tensor):
        ...
        if self.calendar_base == 'weekly':
            calendar_timestamps = (timestamps//(604800))
        elif self.calendar_base == 'monthly':
            calendar_timestamps = (timestamps//(2592000))
        elif self.calendar_base == 'yearly':
            calendar_timestamps = (timestamps//(31536000))
        else:
            calendar_timestamps = timestamps
        
        # coarse-granularity temporal embedding
        calendar_timestamps = calendar_timestamps.unsqueeze(dim=2)
        output_calendar = self.w_calendar(calendar_timestamps)

        # fine-granularity temporal embedding
        timestamps = timestamps.unsqueeze(dim=2)
        output = torch.cos(self.w(timestamps))

        return torch.cat([output, output_calendar],dim=-1)

class DecomposeEncoder(nn.Module):

    def __init__(self, id_dim: int, kernel_size: int, parameter_requires_grad: bool = True):
        """
        Decompose encoder.
        :param dim: int, dimension of decomposition encodings
        :param kernel_size: int, kernel size of moving average
        :param parameter_requires_grad: boolean, whether the parameter in DecomposeEncoder needs gradient
        """
        super(DecomposeEncoder, self).__init__()

        self.id_dim = id_dim
        # seasonality & trend decomposition 
        self.decomp = series_decomp(kernel_size)
        self.season_linear = nn.Linear(1, int(id_dim/2))
        self.trend_linear = nn.Linear(1, int(id_dim/2))


    def forward(self, ids: torch.Tensor):
        """
        compute decomposition encodings
        :param ids: Tensor
        :return:
        """
        ids = ids.unsqueeze(dim=2)
        seasonal, trend = self.decomp(ids)
        seasonal = self.season_linear(F.normalize(seasonal))
        trend = self.trend_linear(F.normalize(trend))
...
class series_decomp(nn.Module):
    ...
    def forward(self, x):
        # smoothed overall trend
        moving_mean = self.moving_avg(x)
        # filtered seasonal variation
        res = x - moving_mean
...
class moving_avg(nn.Module):
    ...
    def forward(self, x):
        # padding on the both ends of the sequence
...
class MergeLayer_v2(nn.Module):
    def __init__(self, input_dim1: int, input_dim2: int, input_dim3: int, hidden_dim: int, output_dim: int, use_BIE_feature: bool):
        """
        Merge Layer to merge two inputs via: input_dim1 + input_dim2 -> hidden_dim -> output_dim.
        :param input_dim1: int, dimension of first input
        :param input_dim2: int, dimension of the second input
        ...
        super().__init__()
        if use_BIE_feature:
            self.BIE_feature = input_dim3
        else:
            self.BIE_feature = 0
        self.fc1 = nn.Linear(input_dim1 + input_dim2 + self.BIE_feature, hidden_dim)
        self.fc2 = nn.Linear(hidden_dim, output_dim)
        self.act = nn.ReLU()
